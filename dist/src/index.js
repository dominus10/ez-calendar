...
const DateSelectorFactory = (() => {
    class DateSelector extends HTMLElement {
        ...
        get value() {
            return this.selectedDay
                ? `${this.yearSelect.value}-${String(parseInt(this.monthSelect.value) + 1).padStart(2, "0")}-${String(this.selectedDay).padStart(2, "0")}`
                : "";
        }
        set value(newValue) {
    ...
    return () => document.createElement("date-selector");
})();
class Calendar extends HTMLElement {
    constructor() {
        ...
        this.dateSelector.addEventListener("click", (event) => event.stopPropagation());
